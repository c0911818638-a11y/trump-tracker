...
(async () => {
  ...
  const context = await browser.newContext({
    ...
    extraHTTPHeaders: { Authorization: `Bearer ${BEARER_TOKEN}` }
  });
  ...
  await page.goto(ACCOUNT_URL, { waitUntil: 'networkidle', timeout: 60000 });
  await page.waitForTimeout(3000);
  await browser.close();

  console.log(`Captured ${posts.length} posts from API`);
